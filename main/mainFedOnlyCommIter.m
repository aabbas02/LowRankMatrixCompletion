...
saveName = "Comm_n_" + num2str(n) + "_q_" + num2str(q) + "_r_" + num2str(r) + "_p_"+...
...
for mc = 1 : MC
    ...
    % --- AltGD (Federated)
    kAltGD = 2;
    [SDAltGDFed(mc,:), timeAltGDFed(mc,:), timeCommAltGDFed(mc,:)] = factGDEta(Xzeros,r,Ustr,kAltGD*T,p,...
    ...
    % --- AltMin (Parfor)
    [SDAltMinParfor(mc,:), timeAltMinParfor(mc,:), timeCommAltMinParfor(mc,:)] = altMinParfor_T(Xzeros,r,p, ...
									                              Ustr,T, ...
									                              rowIdx,Xcol,colIdx,Xrow,numWrkrs,Tsvd);  
    
    % --- AltMin (Fed./Prvt.)
    T_inner =  10;
    [SDAltMinPrvt(mc,:), timeAltMinPrvt(mc,:), timeCommAltMinPrvt(mc,:)] = altMinPrvt_T(Xzeros,r,p, ...
                                                               Ustr,T, ...
    ...
    kAltGDMin = 2;
    eta_c = 1.00;
    [SDAltGDMineta1(mc,:),timeAltGDMineta1(mc,:),~,timeCommAltGDMineta1(mc,:)] = altGDMin_T(r,eta_c, ...
...
toc
[sum(timeAltGDFed(:,50))/mc sum(timeCommAltGDFed(:,50))/mc]
[sum(timeAltMinPrvt(:,50))/mc sum(timeCommAltMinPrvt(:,50))/mc]
[sum(timeAltMinParfor(:,50))/mc sum(timeCommAltMinParfor(:,50))/mc]
[sum(timeAltGDMineta1(:,50))/mc sum(timeCommAltGDMineta1(:,50))/mc]

save(saveName,...
    "SDAltMinPrvt","timeAltMinPrvt", "timeCommAltMinPrvt",...
    "SDAltGDMineta1","timeAltGDMineta1", "timeCommAltGDMineta1",...
    "SDAltMinParfor","timeAltMinParfor", "timeCommAltMinParfor",...
    "SDAltGDFed","timeAltGDFed","timeCommAltGDFed",...
...
timeCommAltMinParfor_ = zeros(1e3,T+1);
%
SDAltMinPrvt_ = zeros(1e3,T+1);
timeAltMinPrvt_ = zeros(1e3,T+1);
timeCommAltMinPrvt_ = zeros(1e3,T+1);
%
...
timeCommAltGDFed_ = zeros(1e3,2*T+1);
%
...
timeCommAltGDMineta1_ = zeros(1e3,2*T+1);
% --
strt = 1;
for i = 1 : numData
    %load("n_"+num2str(n)+"_q_"+num2str(q)+"_r_"+num2str(r)+"_p_"+num2str(p),...
    ...
    timeCommAltMinParfor_(strt:strt+mc-1,:) = timeCommAltMinParfor;
    %
    SDAltMinPrvt_(strt:strt+mc-1,:) = SDAltMinPrvt;
    timeAltMinPrvt_(strt:strt+mc-1,:) = timeAltMinPrvt;
    timeCommAltMinPrvt_(strt:strt+mc-1,:) = timeCommAltMinPrvt;
    %
    ...
    timeCommAltGDFed_(strt:strt+mc-1,:) = timeCommAltGDFed;
    %
    ...
    timeCommAltGDMineta1_(strt:strt+mc-1,:) = timeCommAltGDMineta1;
    % --
    strt = strt + mc;
end
...
timeCommAltMinParfor = timeCommAltMinParfor_(1:MC,:);
% --
SDAltMinPrvt = SDAltMinPrvt_(1:MC,:);
timeAltMinPrvt = timeAltMinPrvt_(1:MC,:);
timeCommAltMinPrvt = timeCommAltMinPrvt_(1:MC,:);
% --
...
timeCommAltGDFed = timeCommAltGDFed_(1:MC,:);
% --
...
timeCommAltGDMineta1 = timeCommAltGDMineta1_(1:MC,:);
%---
% 1
SDAltMinParfor = sum(SDAltMinParfor,1)/MC;
timeAltMinParfor = sum(timeAltMinParfor,1)/MC;
timeCommAltMinParfor = sum(timeCommAltMinParfor,1)/MC;
% 3
SDAltMinPrvt = sum(SDAltMinPrvt,1)/MC;
timeAltMinPrvt = sum(timeAltMinPrvt,1)/MC;
timeCommAltMinPrvt = sum(timeCommAltMinPrvt,1)/MC;
%
%SDAltGDFedHalf = sum(SDAltGDFedHalf,1)/MC;
%timeAltGDFedHalf = sum(timeAltGDFedHalf,1)/MC;
% 5
SDAltGDFed = sum(SDAltGDFed,1)/MC;
timeAltGDFed = sum(timeAltGDFed,1)/MC;
timeCommAltGDFed = sum(timeCommAltGDFed,1)/MC;
%
%SDAltGDFed1 = sum(SDAltGDFed1,1)/MC;
%timeAltGDFed1 = sum(timeAltGDFed1,1)/MC;
% 7
...
timeCommAltGDFed = downsample(timeCommAltGDFed,4);
%
...
timeCommAltGDMineta1 = downsample(timeCommAltGDMineta1,5);
%timeSD = 36;
figure % SD vs Total time
semilogy(timeAltGDMineta1(timeAltGDMineta1 <= timeSD ),...
...
ylabel('$\mathrm{SD}(\mathbf{U}^{(t)}, \mathbf{U}^*)$','Interpreter','Latex','Fontsize',14.0);
...
title("Total Time. n = "  + n +  ", q = " + q +...
    ", r = " + r +  ", p = " + p + ", Workers =" + numWrkrs+".",...
    'Interpreter', 'Latex', 'FontSize',14)
cores = feature('numCores');
stringTitle = ['TotalTime_Fed_Wrkrs_',num2str(numWrkrs),'Max',num2str(cores),'_MC_',num2str(MC),...
    '_n_',num2str(n),'_q_',num2str(q),'_r_',num2str(r),'_p_',num2str(p),...
    'T_',num2str(T),'_id',num2str(randi(1e3,1))];
exportgraphics(gcf,[stringTitle,'.pdf'])
cd(dir)
%--------------------------------------------
figure % SD vs Comm. Time
semilogy(timeCommAltGDMineta1(timeAltGDMineta1 <= timeSD ),...
    SDAltGDMineta1(timeAltGDMineta1 <= timeSD ),'DisplayName', ...
    'AltGDMin(Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','square','MarkerSize',9, 'Color',"#0000FF")
hold on
semilogy(timeCommAltMinParfor(timeAltMinParfor <= timeSD ),...
    SDAltMinParfor(timeAltMinParfor <= timeSD),'DisplayName', ...
    'AltMin(Fed./NotPrvt.)', ...
    'LineWidth',1.35,'Marker','x','MarkerSize',9,'Color',"#D95319")
semilogy(timeCommAltMinPrvt(timeAltMinPrvt <= timeSD ),...
    SDAltMinPrvt(timeAltMinPrvt <= timeSD),'DisplayName', ...
    'AltMin(Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','*','MarkerSize',9,'Color',"#EDB120")
...
%      'LineWidth',1.35,'Marker','diamond','MarkerSize',9,'Color',"0.72,0.27,1.00")
%-
semilogy(timeCommAltGDFed(timeAltGDFed <= timeSD ),...
    SDAltGDFed(timeAltGDFed <= timeSD),'DisplayName', ...
    'FactGD (Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','diamond','MarkerSize',9,'Color',"#7E2F8E")
%-
...
%yHndl.Position(2) = yHndl.Position(2) - 0.05*yHndl.Position(2);
xlabel('t/seconds','Fontsize',13,'Interpreter','Latex')
title("Comm. Time. n = "  + n +  ", q = " + q +...
    ", r = " + r +  ", p = " + p + ", Workers =" + numWrkrs+".",...
    'Interpreter', 'Latex', 'FontSize',14)
cores = feature('numCores');
stringTitle = ['CommTime_Fed_Wrkrs_',num2str(numWrkrs),'Max',num2str(cores),'_MC_',num2str(MC),...
    '_n_',num2str(n),'_q_',num2str(q),'_r_',num2str(r),'_p_',num2str(p),...
    'T_',num2str(T),'_id',num2str(randi(1e3,1))];
cd(dir)
exportgraphics(gcf,[stringTitle,'.pdf'])
...
%----------------------
%----------------------
% figure Iter. vs Total Time
figure
plot(2*(1:length(timeAltGDMineta1 )),...
    timeAltGDMineta1,'DisplayName', ...
    'AltGDMin(Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','square','MarkerSize',9, 'Color',"#0000FF")
hold on
plot(1:length((timeAltMinParfor)),...
    (timeAltMinParfor),'DisplayName', ...
    'AltMin(Fed./NotPrvt.)', ...
    'LineWidth',1.35,'Marker','x','MarkerSize',9,'Color',"#D95319")
plot(1:length((timeAltMinPrvt)),...
    timeAltMinPrvt,'DisplayName', ...
    'AltMin(Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','*','MarkerSize',9,'Color',"#EDB120")
%-
%semilogy(timeAltGDFedHalf(timeAltGDFedHalf <= timeSD ),...
...
%      'LineWidth',1.35,'Marker','diamond','MarkerSize',9,'Color',"0.72,0.27,1.00")
%-
plot(4*(1:length( (timeAltGDFed) )),...
    timeAltGDFed,'DisplayName', ...
    'FactGD (Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','diamond','MarkerSize',9,'Color',"#7E2F8E")
%-
%semilogy(timeAltGDFed1(timeAltGDFed1 <= timeSD ),...
...
grid on
legend('Interpreter','Latex','Fontsize',9.25, 'Location','Northeast')
ylabel('$time/s$','Interpreter','Latex','Fontsize',14.0);
%yHndl = ylabel('$\mathrm{SD}(\mathbf{U}^{(t)}, \mathbf{U}^*)$','Interpreter','Latex','Fontsize',14.0);
%yHndl.Position(1) = yHndl.Position(1) + 1.25;
%yHndl.Position(2) = yHndl.Position(2) - 0.05*yHndl.Position(2);
xlabel('Ieration no.','Fontsize',13,'Interpreter','Latex')
title("Total. Time. n = "  + n +  ", q = " + q +...
    ", r = " + r +  ", p = " + p + ", Workers =" + numWrkrs+".",...
    'Interpreter', 'Latex', 'FontSize',14)
cores = feature('numCores');
stringTitle = ['Iter_TotalTime_Fed_Wrkrs_',num2str(numWrkrs),'Max',num2str(cores),'_MC_',num2str(MC),...
    '_n_',num2str(n),'_q_',num2str(q),'_r_',num2str(r),'_p_',num2str(p),...
    'T_',num2str(T),'_id',num2str(randi(1e3,1))];
cd(dir)
exportgraphics(gcf,[stringTitle,'.pdf'])
% figure Iter. vs Comm. Time
figure
plot(2*(1:length(timeCommAltGDMineta1)), ...
    timeCommAltGDMineta1,'DisplayName', ...
    'AltGDMin(Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','square','MarkerSize',9, 'Color',"#0000FF")
hold on
plot(1:length((timeCommAltMinParfor)),...
    timeCommAltMinParfor,'DisplayName', ...
    'AltMin(Fed./NotPrvt.)', ...
    'LineWidth',1.35,'Marker','x','MarkerSize',9,'Color',"#D95319")
plot(1:length((timeCommAltMinPrvt)), timeCommAltMinPrvt,'DisplayName', ...
    'AltMin(Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','*','MarkerSize',9,'Color',"#EDB120")
plot(4*(1:length(timeCommAltGDFed)),...
    timeCommAltGDFed,'DisplayName', ...
    'FactGD (Fed./Prvt.)', ...
    'LineWidth',1.35,'Marker','diamond','MarkerSize',9,'Color',"#7E2F8E")
grid on
legend('Interpreter','Latex','Fontsize',9.25, 'Location','Northeast')
...
%yHndl.Position(1) = yHndl.Position(1) + 1.25;
%yHndl.Position(2) = yHndl.Position(2) - 0.05*yHndl.Position(2);
xlabel('Iteration No.','Fontsize',13,'Interpreter','Latex')
title("Comm. Time. n = "  + n +  ", q = " + q +...
    ", r = " + r +  ", p = " + p + ", Workers =" + numWrkrs+".",...
    'Interpreter', 'Latex', 'FontSize',14)
cores = feature('numCores');
stringTitle = ['Iter_CommTime_Iter_Fed_Wrkrs_',num2str(numWrkrs),'Max',num2str(cores),'_MC_',num2str(MC),...
    '_n_',num2str(n),'_q_',num2str(q),'_r_',num2str(r),'_p_',num2str(p),...
    'T_',num2str(T),'_id',num2str(randi(1e3,1))];
cd(dir)
exportgraphics(gcf,[stringTitle,'.pdf'])
...
timeArr = zeros(T + 1, 1);
timeComm = zeros(T + 1, 1);
tWrkrs1 = zeros(numWrkrs, 1);
tWrkrs2 = zeros(numWrkrs,1);
for i = 1 : T
    tStartFed1 = tic;
    parfor j = 1 : numWrkrs
        tStartWrkr = tic;
        ...
        T3_(:,:,j) = V_{j}'*V_{j};
        tWrkrs1(j) = toc(tStartWrkr);
    end
    tEndFed1 = toc(tStartFed1) - max(tWrkrs1);
    ...
    parfor j = 1 : numWrkrs
        ...
        tWrkrs2(j) = toc(tStartWrkr);
    end
    tEndFed2 = toc(tStartFed2) - max(tWrkrs2);
    timeArr(i+1) = timeArr(i) + toc(tStart);
    timeComm(i+1) = timeComm(i) + tEndFed1 + tEndFed2;
