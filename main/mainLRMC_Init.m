...
SDU0 = zeros(length(numBlocksTry_),MC); SDU0_ = zeros(length(numBlocksTry_),MC);
SDU0Cllps = zeros(length(numBlocksTry_),MC);  SDU0Cllps_ = zeros(length(numBlocksTry_),MC); 
...
%fill = "mean"; %fill = "median"; %fill = "both";
fill = "one";
%-----------------------
...
for i = 1 : length(numBlocksTry_)
    ...
    for mc = 1 : MC
        ...
        T_init = 0; % skip iterations by setting T_init = 0
        [SDVals,Errs] = altMinwithP_LRMC(n,q, r, U0, Ustr, T_init, rowIdx, Xcol, colIdx, Xrow, X0,idx,Xzeros,real);
        ...
        SDU0Cllps(i,mc) = norm(Ustr - U0Cllps*(U0Cllps'*Ustr));
        SDInit = norm(Ustr - Uproj*(Uproj'*Ustr))
        idxFlag = 0;
        T = 100;
        [XzerosPerm, rowIdxPerm, colIdxPerm, XcolPerm, XrowPerm,~] = processMatrix(XzerosPerm, n, q, p,real,idxFlag,idx);        
        [SDVals,~,~] = minCllpsInit(n,q,r,r_All,U0Cllps, ...
                                  Ustr,Bstr,T, ...
                                  rowIdxPerm,XcolPerm, 0,0,0,0);   
        SDU0Cllps_(i,mc) = SDVals(end);        
        mc
    end
end
SDU0 = sum(SDU0,2)/MC; SDU0_ = sum(SDU0_,2)/MC;
SDU0Cllps = sum(SDU0Cllps,2)/MC; SDU0Cllps_ = sum(SDU0Cllps_,2)/MC;
