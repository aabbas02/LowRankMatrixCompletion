function [SDVals,objVals,Uproj] = minCllpsInit(n,q,r,r_,Uinit, ...
                                               Ustr,Bstr,T, ...
                                               rowIdx,Xcol, Xhat0,idx,Xzeros,real,T_in)

% why gradient descent for U on the collapsed objective instead of least-squares?
% because, the collapsed matrix is different across all columns. 
% chain Rule: https://tutorial.math.lamar.edu/classes/calciii/chainrule.aspx
% computes gradient of U with respect to each of the q columns
% for AltMin, T_in > 1, where T_in is the number of GD updates for U with B
% fixed
[Vstr,~,~] = qr(Bstr','econ');
U = Uinit;
SDVals = zeros(T+1,1);
[Uproj,~,~] = qr(U,'econ');
SDVals(1) =  norm(Ustr - Uproj*(Uproj'*Ustr));
SDVals(1)
objVals = zeros(T+1,1);
% Step - size calculation based off of operator norm of Xinit
Xinit = zeros(n,q);
numObserved = 0;
Bhat = zeros(r,q);
for k =  1 : q
    Ucllpsk = zeros(length(r_{k}),r);
    XcolCllpsk = zeros(length(r_{k}),1);
    %1. form collapsed matrix Ucllpsk and collapsed Xcolk
    start = 1;
    for i = 1 : length(r_{k})
        blkSize = r_{k}(i);
        rowsBlock = rowIdx{k}(start: start + blkSize - 1);            
        Ucllpsk(i,:) = sum(U(rowsBlock,:));
        XcolCllpsk(i) = sum(Xcol{k}(start: start + blkSize - 1));
        start = start  + blkSize;
        % Option A for X init / step size selection
        numObserved = numObserved + 1;
        Xinit(rowsBlock(1),k) = XcolCllpsk(i);
    end
    % Option B for X init / step size selection
    %numObserved = length(Xcol{k});
    %2. least - squares update
    %Xinit(rowIdx{k},k) = U(rowIdx{k},:)*(pinv(Ucllpsk)*XcolCllpsk);
    % OR
    %Xinit(rowIdx{k},k) = Xcol{k};     
end
p = numObserved/(n*q);
[~,S] = svds(Xinit/p);
eta = 0.1/(S(1,1)^2*p);
gradU = zeros(n,r);
B = zeros(r,q);
for t = 1 : T
    gradU = 0*gradU;
    err = 0;
    errB = 0;
    for k =  1 : q
        Ucllpsk = zeros(length(r_{k}),r);
        XcolCllpsk = zeros(length(r_{k}),1);
        %1. form collapsed matrix Ucllpsk and collapsed Xcolk
        start = 1;
        for i = 1 : length(r_{k})
            blkSize = r_{k}(i);
            rowsBlock = rowIdx{k}(start: start + blkSize - 1);            
            Ucllpsk(i,:) = sum(U(rowsBlock,:));
            XcolCllpsk(i) = sum(Xcol{k}(start: start + blkSize - 1));
            start = start  + blkSize;
        end
        %2. least - squares update
        if mod(t-1,T_in) == 0 % update at t = 1, ...,T_in + 1, 2*T_in + 1,...
            bk = pinv(Ucllpsk)*XcolCllpsk;
            B(:,k) = bk;   
        end
        bk = B(:,k);
        %3. get gradient with respect to Ucllpsk 
        diffk = Ucllpsk*bk - XcolCllpsk; % vector of cardinality |Omega_k|
        Bhat(:,k) = bk;
        err = err + norm(diffk);
        errB = errB + norm(bk - Bstr(:,k))^2;
        gradk = diffk*bk'; % this is an |Omega_k| x r matrix
        %4. get gradient with respect to  U -  chain rule
        start = 1;
        for i = 1 : length(r_{k})
            blkSize = r_{k}(i);
            rowsBlock = rowIdx{k}(start: start + blkSize - 1);
            gradU(rowsBlock,:) = gradU(rowsBlock,:) + gradk(i,:); % All rows that make up block i, denoted rowsBlock here,
                                                                  % have the same gradient
            start = start + blkSize;
        end
    end
    U = U - eta*gradU;
    [Uproj,~,~] = qr(U,'econ');
    SDVals(t+1) = norm(Ustr - Uproj*(Uproj'*Ustr));    
    [Vproj,~,~] = qr(Bhat','econ');
    SDvalV =  norm(Vstr - Vproj*(Vproj'*Vstr));
    if mod(t,25) == 0 && real == 0
        disp(['Cllps Init.', ' Iter =  ', num2str(t), '. ObjVal = ', num2str(err),  '. Gradient Norm = ', num2str(norm(gradU)), ...
            '. SD U = ', num2str(SDVals(t+1)), ...
            '. SD V = ', num2str(SDvalV),...
            '. rel Err X = ', num2str(norm(Ustr*Bstr - U*Bhat,'fro')/norm(Ustr*Bstr,'fro')),...
            '. rel Err B = ', num2str(errB/norm(Bstr,'fro')^2)])
        if norm(gradU) < 1e-8
            break
        end
    end
end
