...
def plotTimeAgnstWrkrs(numWrkrs_,  n,q,r,p,ID,lim,MC, timeFactGD = -1*np.ones(1000),
    ...
    # Save the figure
    if median:
        fig1.savefig(f"figures/6Nov__tmeAgnstWrkrsMedian_n_{n}_q_{q}_r_{r}_p_{p}_T_in_{T_in}_MC_{MC}_ID_{ID}_.pdf", bbox_inches='tight')    
    else:
        fig1.savefig(f"figures/6Nov_tmeAgnstWrkrsMean_n_{n}_q_{q}_r_{r}_p_{p}_T_in_{T_in}_MC_{MC}_ID_{ID}_.pdf", bbox_inches='tight')
...
def plotErrAgnstTime(numWrkrs,n,q,r,p,ID,lim,MC,
    ...
    plt.semilogy(timeAltMinFullyDist, SDAltMinFullyDist, label='AltMin(Fed.)', marker="s")
