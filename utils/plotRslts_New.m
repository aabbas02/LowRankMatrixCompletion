function plotRslts_New(time_AltGDMin, SDVals_AltGDMin, ...
                   time_AltGDMin_Distributed, SDVals_AltGDMin_Distributed, ...
                   ...
                   n,q,r,m,numBlocks,MC,same,T_LS,eta_c,eta_L,noiseVar, distributed)
    curDir = pwd;
    cd ..
    cd figsMtrxSensing
    % AVERAGING
    ...
    SDVals_AltGDMin_Distributed = sum(SDVals_AltGDMin_Distributed,1)/MC;    
    ...
    time_AltGDMin_Distributed = sum(time_AltGDMin_Distributed,1)/MC;
    ...
    if ~(all (SDVals_AltGDMin_Distributed == 0))
        semilogy(SDVals_AltGDMin_Distributed, ...
            'DisplayName', 'AltGDMin-Distributed', 'LineWidth', 1.45, 'Marker', 'diamond', 'MarkerSize', 7);
        hold on;
    end
    ...
    if ~(all (SDVals_AltGDMin_Distributed == 0))
    semilogy(time_AltGDMin_Distributed, SDVals_AltGDMin_Distributed, ...
        'DisplayName', ' AltGDMin-Distributed', 'LineWidth', 1.45, 'Marker', 'diamond', 'MarkerSize', 7);
    hold on;
    end
    ...
    title("n = " + n + ", q = " + q +...
          ...
          ", $\sigma^2 = $ ", + noiseVar,'Interpreter', 'Latex', 'FontSize',11)
    
    legend('Interpreter', 'Latex', 'Fontsize', 11);
    ylabel("$SD(U^{(t)},U^*)$","FontSize",14,'Interpreter','Latex')
    xlabel('t (Seconds)', 'FontSize',14, 'Interpreter','Latex')
    stringTitle = ['Time_Distributed_MC_', num2str(MC), ...
                   ...
                   '_eta_L_',num2str(eta_L),'noiseVar_',num2str(noiseVar), '_distributed_', num2str(distributed)];
    savefig([stringTitle, '.fig']);    
